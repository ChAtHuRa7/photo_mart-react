...
import './form.css';
  ...
  const [bookingData , setBookingData] = useState({userId:'',photographerId:'',calendarId:'',customerRequestDto:'',eventRequestDto:'',packageRequestDto:''});
  ...
  useEffect(()=>{
      ...
      setBookingData({...bookingData, userId:profile?.userId, photographerId:id.id})
      setCustomerData({...customerData, fullName: profile.userName ,email:profile.userMail ,phoneNo:profile.mobileNumber ,address:profile.address})
  },[location]);

  const handelCalendarData = (data) =>{
    setCalendarData({photographerId:id.id, userId:profile?.userId, dateTime:data, status:'pending'});
  };
    ...
    <div className="booking_form_container">
      ...
      <div className="packages_container">
        <div className="packages_card_container">
            <Grid key={profile?.userId} container direction={'row'} justifyContent={'space-evenly'}>
              {packages?.map(pack=> 
                <Grid key={pack.packageId} item xm={4}>
                  <div className="package_card">
                      <p>{pack.packageTittle}</p>
                    <input type="radio" onChange={(e)=>handelPackageData(e.target.value)} value={pack.packageId} name="type" className="package_radio" />
                  </div>
                </Grid>
              )}
            </Grid>
        ...
        <div className="information_container">
          <div className='textBox_column'>
            ...
            <div className="info_text_box">
              <p>Full Name</p>
              <input value={customerData?.fullName} onChange={(e)=>setCustomerData({...customerData, fullName:e.target.value})} type="text"  name="text" className="text_box" />
            </div>

            <div className="info_text_box">
              <p>Email</p>
              <input value={customerData?.email} onChange={(e)=>setCustomerData({...customerData, email:e.target.value})} type="text"  name="text" className="text_box" />
            </div>
          ...
          <div className='textBox_column'>
            ...
            <div className="info_text_box">
              <p>Mobile</p>
              <input pattern="[0-9]{1}[0-9]{9}" value={customerData?.phoneNo} onChange={(e)=>setCustomerData({...customerData, phoneNo:e.target.value})} type="number"  name="text" className="text_box" />
            </div>

            <div className="info_text_box">
              <p>Address</p>
              <input value={customerData?.address} onChange={(e)=>setCustomerData({...customerData, address:e.target.value})} type="text"  name="text" className="text_box" />
            </div>

            <div className="info_text_box">
              <p>Wedding Start Time</p>
              <input type="time" onChange={(e)=>setEventData({...eventData, time:e.target.value})}  value={eventData?.time}/>
            </div>
